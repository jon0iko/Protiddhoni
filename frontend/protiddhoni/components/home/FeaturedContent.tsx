    ...
    const transformContent = (item: any) => {
        ...
        if (isSeries) {
            return {
                ...
                rating: 0,
                views: 0,
                publishedAt: item.created_at ? new Date(/Z|[+-]\d{2}:\d{2}$/.test(item.created_at) ? item.created_at : item.created_at + 'Z').toLocaleDateString('bn-BD') : '',
            ...
            return {
                ...
                publishedAt: item.published_at ? new Date(/Z|[+-]\d{2}:\d{2}$/.test(item.published_at) ? item.published_at : item.published_at + 'Z').toLocaleDateString('bn-BD') : '',
                readTime: '15 মিনিট',
                status: 'completed',
                chapters: 1,
                tags: [],
