...
describe('POST /api/auth/register', () => {
    ...
    test('returns 400 when the username is already taken', async () => {
        ...
        UserRepository.findByUsername.mockResolvedValue({ id: 'existing-user' });
    ...
    test('creates the user, returns a valid JWT, and never leaks the password hash', async () => {
        ...
        const storedHash = UserRepository.create.mock.calls[0][0].password_hash;
...
describe('GET /api/auth/profile (protected route)', () => {
    ...
    test('returns the profile with a valid token', async () => {
        UserRepository.findById.mockResolvedValue({ id: 'user-1', username: 'reader', email: 'user@b.com' });
        UserRepository.getUserStats.mockResolvedValue({ followers: 3, following: 1 });
