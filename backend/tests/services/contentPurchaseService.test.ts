...
describe('ContentPurchaseService', () => {
    describe('hasPurchased', () => {
        test('returns true when a purchase row exists', async () => {
            (db.getClient as jest.Mock).mockReturnValue({ from: () => mockChain({ data: { id: 'purchase-1' }, error: null }) });
        ...
        test('returns false when no purchase row exists (PGRST116)', async () => {
            (db.getClient as jest.Mock).mockReturnValue({ from: () => mockChain({ data: null, error: { code: 'PGRST116' } }) });
    ...
    describe('purchaseContent', () => {
        ...
        test('returns not-found when the content does not exist', async () => {
            ...
            (db.getClient as jest.Mock).mockReturnValue({ from: () => mockChain({ data: null, error: { message: 'missing' } }) });
